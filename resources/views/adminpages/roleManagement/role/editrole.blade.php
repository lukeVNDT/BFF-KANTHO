  ...
  @if(Session::has('success'))
    ...
    <section class="content">
        ...
        <!-- right column -->
          ...
          <form action="{{ url('/editrole/'.$role->id) }}" method="POST">
            {{csrf_field()}}
             <div class="card">
            ...
            <!-- form start -->
              <div class="card-body">
                <div class="form-group">
                  <input type="text" name="name" class="form-control" id="phone" value="{{ $role->name }}">
                </div>
              </div>
              <!-- /.box-body -->
          </div>
          ...
          <div class="card">
            ...
            <!-- form start -->
              <div class="card-body">
                <div class="form-group">
                  <textarea rows="5" type="text" name="display_name" class="form-control" id="email" >{{ $role->display_name }}</textarea>
                </div>
                                    ...
                                    <div class="row no-gutters align-items-center">
                                      @foreach($parent->hasChild as $child)
                                        <div class="col-md-3 mb-5">
                                          <input
                                            {{ $checkedPermission->contains('id', $child->id) ? "checked" : '' }}
                                           value="{{ $child->id }}" class="checkbox-children" type="checkbox" name="permission_id[]"> {{ $child->name }}
